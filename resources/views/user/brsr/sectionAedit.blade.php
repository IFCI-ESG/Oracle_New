                                ...
                                <div class="tab-pane fade show active" id="social" role="tabpanel" aria-labelledby="social-tab">
                                    <div class="card card-success card-outline">
                                        <div class="card-header">
                                             <b>Details of the listed entity</b>
                                        </div>
                                            ...
                                            <table class="table table-bordered table-hover table-sm table-striped" id="employee_data">
                                                <thead>
                                                    <tr class="text-center table-success">
                                                        ...
                                                        <th style="width: 10%" class="text-center">Response</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @php
                                                    ...
                                                    @foreach ($quesMast->where('question_section','Section A') as $key => $emp)
                                                     @php
                                                        // Get the corresponding social value for the current question
                                                        $social_value_for_current_question = $brsr_value->firstWhere('ques_id', $emp->id);
                                                     ...
                                                     @endphp
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td @if($emp->question_type == 'heading') style="font-size: 1rem; font-weight: bold;" colspan="5" @else style="font-size: 1rem;" @endif>
                                                                {{$emp->question}}
                                                            ...
                                                            @if ($emp->id == 4 || $emp->id == 5)
                                                                <td>
                                                                    <textarea style="text-align: left; width: 500px;" name="emp[{{ $a }}][response]" class="form-control form-control-sm" required>{{ $social_value_for_current_question ? $social_value_for_current_question->response : '' }}</textarea>
                                                                </td>
                                                            @elseif ($emp->id == 1)
                                                                <td>
                                                            ...
                                                            @elseif ($emp->id == 13)
                                                                <td>
                                                                    <textarea style="text-align: left; width: 500px;height:200px;" name="emp[{{ $a }}][response]" class="form-control form-control-sm" required>{{ $social_value_for_current_question ? $social_value_for_current_question->response : '' }}</textarea>
                                                                </td>
                                                            @else
                                                                <td>
                                                                    <input type="text" style="text-align: left;width:500px;" name="emp[{{ $a }}][response]" class="form-control form-control-sm" value="{{ $social_value_for_current_question ? $social_value_for_current_question->response : '' }}" required>
                                                                </td>
                                                            @endif
                                                        </tr>
                                                        @php
         ...
         <table class="table table-bordered table-hover table-sm table-striped" id="additional_data_table">
            ...
            <tbody id="additional_data_rows">
                @php
                    $rowCount = 1;
                @endphp
                @foreach($brsr_prod_serv_value as $key => $productService)
                    <tr>
...
<!-- End of new Table -->
         ...
         <!-- Products/Services NIC Activity -->
        <div class="card card-success card-outline mt-4">
    <div class="card-header">
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="additional_data_table">
            ...
            <tbody id="additional_data_rows">
                @php
                    $rowCount = 1;
                @endphp
                @foreach($brsr_nic_prod_serv_value as $key => $productService)
                    <tr>
                        <td class="text-center">{{ $key + 1 }} </td>
    ...
    <div class="card-body p-3">
        <table class="table table-bordered table-hover table-sm table-striped" id="operations_data_table">
            ...
            @php
                 $rowCount = 1;
                @endphp
                @foreach($brsr_operations_value as $key => $operation)
                <tr>
        ...
        <table class="table table-bordered table-hover table-sm table-striped" id="operations_data_table1">
            ...
            <tbody id="operations_data_rows1">
                ...
                <tr>
                    ...
                    <td><input type="text" class="form-control form-control-sm" name="operation[{{ $rowCount }}][text_i]" value="National (No. of States)" readonly></td>
                    <td><input type="text" class="form-control form-control-sm" name="operation[{{ $rowCount }}][text_j]" value="{{ $operation->national_state_number }}" required ></td>
                </tr>
                 <tr>
                    <td><input type="text" class="form-control form-control-sm" name="operation[{{ $rowCount }}][text_k]" value="International (No. of Countries)" readonly></td>
                    <td><input type="text" class="form-control form-control-sm" name="operation[{{ $rowCount }}][text_l]" value="{{ $operation->international_country_number }}" required ></td>
                 </tr>
...
<!-- Operations - contribution of exports -->
 <div class="card card-success card-outline mt-4">
    ...
    <div class="card-body p-1">
        <textarea class="form-control form-control-sm" name="operation[{{ $rowCount }}][text_m]" placeholder="Enter the contribution of exports as a percentage of total turnover" rows="6" required>{{ $operation->export_contribution }}</textarea>
    </div>
</div>
...
<!-- End of Operations-  contribution of exports Table -->
 ...
 <div class="card card-success card-outline mt-4">
    ...
    <div class="card-body p-1">
        <textarea class="form-control form-control-sm" name="operation[{{ $rowCount }}][text_n]" placeholder="Enter brief on types of customers " rows="6" required>{{ $operation->customer_brief }}</textarea>
    </div>
    @php
      $rowCount++;
    @endphp
    ...
    <div class="card-body p-3">
        <table class="table table-bordered table-hover table-sm table-striped" id="employee_data">
            ...
            <tbody>
                ...
                @php
                   $employee_value_for_current_question = $brsr_value->firstWhere('ques_id', $emp->id);
                @endphp
                <tr>
                    ...
                    @else
                        ...
                        <!-- Male (B) -->
                        <td class="text-center">
                            <input type="number" style="text-align: right; width: 120px;" min="0" class="form-control form-control-sm emp_{{$a}} m_emp" name="emp[{{ $a }}][emp_male]" id="emp_male_{{$a}}"  value = '{{ $employee_value_for_current_question->male_count ?? 0}}'
                        ...
                        <!-- Female (C) -->
                        <td class="text-center">
                            <input type="number" style="text-align: right; width: 120px;" min="0" class="form-control form-control-sm emp_{{$a}} f_emp" name="emp[{{ $a }}][emp_female]" id="emp_female_{{$a}}" value = '{{ $employee_value_for_current_question->female_count ?? 0}}' 
                                oninput="calculatePercentages({{ $a }})" required
                                @if(in_array($emp->id, [19, 23, 27, 31])) readonly @endif>
                        </td>
                        <td class="text-center">
    ...
    <div class="card-body p-3">
        <table class="table table-bordered table-hover table-sm table-striped" id="employee_data">
            ...
            <tbody>
                ...
                @php
                   $part_value_for_current_question = $brsr_value->firstWhere('ques_id', $emp->id);
                @endphp
                <tr>
                    ...
                    @else
                      ...
                      <td style="font-size: 1rem;">
                        ...
                        <!-- Total (A) -->
                        <td class="text-center">
                            <input type="number" style="text-align: right; width: 200px;" min="0" class="form-control form-control-sm emp_{{$a}} t_emp" name="emp[{{ $a }}][total_part]" id="total_part_{{$a}}" value = '{{ $part_value_for_current_question->total_part ?? 0}}'
                                oninput="calculatePercentages_participation({{ $a }})" required>
                        </td>

                        <!-- Male (B) -->
                        <td class="text-center">
                            <input type="number" style="text-align: right; width: 200px;" min="0" class="form-control form-control-sm emp_{{$a}} m_emp" name="emp[{{ $a }}][emp_part]" id="emp_part_{{$a}}" value = '{{ $part_value_for_current_question->emp_part ?? 0}}'
                                oninput="calculatePercentages_participation({{ $a }})" required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align:left;width: 200px;" id="male_percent_{{$a}}" class="form-control form-control-sm" readonly>
                        </td>
                      @php
                        $a++;
                    @endphp
    ...
    <div class="card-body p-3">
        <table class="table table-bordered table-hover table-sm table-striped" id="employee_data">
            ...
            <tbody>
                ...
                @php
                   $turnover_value_for_current_question = $brsr_turnover_value->firstWhere('ques_id', $emp->id);
                @endphp
                <tr>
                    ...
                    @else
                      ...
                      <td style="font-size: 1rem;">
                            {{$emp->question}}
                            <input type="hidden" value="{{ $brsr_turnover_value->where('ques_id',$emp->id)->first()->id }}" name="emps[{{ $a }}][row_id]">
                       </td>

                        <!--(Turnover rate in current FY) -->
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;" class="form-control form-control-sm emps_{{$a}} t_emp" name="emps[{{ $a }}][current_turnover_male]" id="current_turnover_male_{{$a}}" value = '{{ $turnover_value_for_current_question->current_turnover_male ?? NA}}' 
                                 required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;"  class="form-control form-control-sm emps_{{$a}} t_emp" name="emps[{{ $a }}][current_turnover_female]" id="current_turnover_female_{{$a}}" value = '{{ $turnover_value_for_current_question->current_turnover_female ?? NA}}'
                                 required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;" class="form-control form-control-sm emps_{{$a}} t_emp" name="emps[{{ $a }}][current_turnover_total]" id="current_turnover_total_{{$a}}" value = '{{ $turnover_value_for_current_question->current_turnover_total ?? NA}}'
                                 required>
                        </td>

                        <!-- (Turnover rate in previous FY) -->
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;" class="form-control form-control-sm emps_{{$a}} m_emp" name="emps[{{ $a }}][previous_turnover_male]" id="previous_turnover_male_{{$a}}" value = '{{ $turnover_value_for_current_question->previous_turnover_male ?? NA}}' 
                                required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;"  class="form-control form-control-sm emps_{{$a}} m_emp" name="emps[{{ $a }}][previous_turnover_female]" id="previous_turnover_female_{{$a}}" value = '{{ $turnover_value_for_current_question->previous_turnover_female ?? NA}}'
                                required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;" class="form-control form-control-sm emps_{{$a}} m_emp" name="emps[{{ $a }}][previous_turnover_total]" id="previous_turnover_total_{{$a}}" value = '{{ $turnover_value_for_current_question->previous_turnover_total ?? NA}}' 
                                required>
                        </td>
                        ...
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;" class="form-control form-control-sm emp_{{$a}} m_emp" name="emps[{{ $a }}][priorprev_turnover_male]" id="priorprev_turnover_male_{{$a}}" value = '{{ $turnover_value_for_current_question->priorprev_turnover_male ?? NA}}'  
                                required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;" class="form-control form-control-sm emp_{{$a}} m_emp" name="emps[{{ $a }}][priorprev_turnover_female]" id="priorprev_turnover_female_{{$a}}" value = '{{ $turnover_value_for_current_question->priorprev_turnover_female ?? NA}}'  
                                required>
                        </td>
                        <td class="text-center">
                            <input type="text" style="text-align: left; width: 70px;"  class="form-control form-control-sm emp_{{$a}} m_emp" name="emps[{{ $a }}][priorprev_turnover_total]" id="priorprev_turnover_total_{{$a}}" value = '{{ $turnover_value_for_current_question->priorprev_turnover_total ?? NA}}'
                                required>
                        </td>
          ...
          <div class="card-header">
               ...
               <tbody id="holding_data_rows">
                @php
                   $rowCount = 1;
                @endphp
                @foreach($brsr_holding_value as $key => $holding)
                 <tr>
                 <td class="text-center">{{ $key + 1 }} </td>
        ...
        <!-- CSR Applicable - Start -->
           <div class="card card-success card-outline mt-4">
             @php
               $rowCount = 1;
             @endphp
             @foreach($brsr_holdings_value as $key => $holdings)
            <div class="card-header">
            <input type="hidden" value="{{ $brsr_holdings_value->where('id',$holdings->id)->first()->id }}" name="holdings[{{ $rowCount }}][row_id]">
             <b><u>CSR Details</u></b><br /><br />
             <b>24.  (i) Whether CSR is applicable as per section 135 of Companies Act, 2013: (Yes/No) </b>
            </div>
             <div class="card-body p-1">
                <textarea class="form-control form-control-sm" name="holdings[{{ $rowCount }}][text_e]" placeholder="" rows="6" required>{{ $holdings->csr_applicable }}</textarea>
            </div>
         </div>
         ...
         <div class="card card-success card-outline mt-4">
            <div class="card-header">
             ...
             <div class="card-body p-1">
                <textarea class="form-control form-control-sm" name="holdings[{{ $rowCount }}][text_f]" placeholder="" rows="6" required>{{ $holdings->turnover_rs }}</textarea>
            </div>
         </div>
         ...
         <div class="card card-success card-outline mt-4">
            <div class="card-header">
             ...
             <div class="card-body p-1">
                <textarea class="form-control form-control-sm" name="holdings[{{ $rowCount }}][text_g]" placeholder="" rows="6" required>{{ $holdings->net_worth }}</textarea>
            </div>
            @php
                $rowCount++;
              @endphp
    ...
    <div class="card-body p-3">
        <table class="table table-bordered table-hover table-sm table-striped" id="employee_data">
            ...
            <tbody>
                ...
                @php
                   $compliace_value_for_current_question = $brsr_compliace_value->firstWhere('ques_id', $emp->id);
                @endphp
                <tr>
  ...
  <div class="card card-success card-outline mt-4">
          <div class="card-header">
               ...
               @php
                   $rowCount = 1;
                @endphp
                @foreach($brsr_material_value as $key => $material)
                 <tr>
                 <td class="text-center">{{ $key + 1 }} </td>
...
<script>
     $(document).ready(function() {
        setTimeout(function() {
                $("#totalemp_16").trigger('onkeyup');
                $("#totalemp_17").trigger('onkeyup');
                $("#totalemp_18").trigger('onkeyup');
                $("#totalemp_19").trigger('onkeyup');
                $("#totalemp_20").trigger('onkeyup');
                $("#totalemp_21").trigger('onkeyup');
                $("#totalemp_22").trigger('onkeyup');
                $("#totalemp_23").trigger('onkeyup');
                $("#totalemp_24").trigger('onkeyup');
                $("#totalemp_25").trigger('onkeyup');
                $("#totalemp_26").trigger('onkeyup');
                $("#totalemp_27").trigger('onkeyup');
                $("#total_part_32").trigger('oninput');
                $("#total_part_33").trigger('oninput');
            }, 2000)
     });
...
<script>
   $(document).ready(function() {
     let rowCount = $('#additional_data_rows tr').length + 1;
...
<script>
    function calculateTotal(rowId) {
        const plants = parseFloat(document.querySelector(`input[name='operation[${rowId}][text_b]']`).value) || 0;
        const offices = parseFloat(document.querySelector(`input[name='operation[${rowId}][text_c]']`).value) || 0;
        const total = plants + offices;
        document.querySelector(`#total_${rowId}`).value = total;
    }

    function calculateTotals(rowId) {
        const plants = parseFloat(document.querySelector(`input[name='operation[${rowId}][text_f]']`).value) || 0;
        const offices = parseFloat(document.querySelector(`input[name='operation[${rowId}][text_g]']`).value) || 0;
        const total = plants + offices;
        document.querySelector(`#totals_${rowId}`).value = total;
    }
</script>

...
class CorporateCarbonFootprintController extends Controller
{

    public function index()
    {
        $fys = DB::table('fy_masters')->where('status', '1')->orderby('id','desc')->get();
        // dd($bank_details);
        return view('admin.corp_carbon_footprint.index', compact('fys'));
    ...
    public function create($fy_id)
    {
        $fy_id = decrypt($fy_id);
            // dd($fy_id);
        // $module_mast = ModuleMast::where('module_type', 'CORP_CARBON_FOOTPRINT')->where('com_id', Auth::user()->id)->where('fy_id',$fy_id)->first();
        // if(!$module_mast)
        // {
        ...
        //         $module_mast->module_type = 'CORP_CARBON_FOOTPRINT';
        // }
        // DB::transaction(function () use ($fy_id,$module_mast)
        // {
        //     $module_mast->save();
        // });
        ...
        $busi_mast = DB::table('BUSINESS_ACTIVITY_MASTER')->where('SECTOR_ID', Auth::user()->sector_id)->where('QUES_TYPE_ID',Auth::user()->comp_type_id)->orderby('ID')->get();

        $busi_value = DB::table('business_activity_value as bav')
                            ->join('BUSINESS_ACTIVITY_MASTER as bam','bam.ID','=','bav.ACTIVITY_ID')
                            ->where('bav.com_id',Auth::user()->id)
                            ->where('bav.fy_id',$fy_id)
                            ->orderby('bav.id')
                            ->get(['bav.*','bam.ACTIVITY']);
                            

        $scope_mast = DB::table('SCOPEMASTER')->where('status', '1')->orderby('ID')->get();
            // dd($busi_value,$busi_mast,Auth::user()->sector_id,Auth::user()->comp_type_id);
        ...
        $seg_mast = DB::table('SEGMENTMASTER as sgm')
                            ...
                            ->where('ssm.ques_type_id',Auth::user()->comp_type_id)
                            ->where('ssm.sector_id',Auth::user()->sector_id)
        ...
        return view('admin.corp_carbon_footprint.create', compact('ques_mast','fy_id','ques_value',
        'sector','fys','seg_mast','busi_mast','busi_value','seg_tot','ques_tot','data_quality',
        'data_qual_value','scope_mast'));
